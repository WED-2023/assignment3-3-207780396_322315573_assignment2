...
router.beforeEach((to, from, next) => {
  ...
  if (to.matched.some(record => record.meta.requiresAuth)) {
    ...
    if (!store.username) {
      ...
      next({
        path: '/login',
        query: { redirect: to.fullPath } // Save the attempted route
      });
    } else {
...
axios.interceptors.response.use((response) => response, (error) => {
  ...
  if (error.response && error.response.status === 401) {
    // User is not authenticated, clear store and redirect to login
    store.logout();
...
// Initialize Bootstrap components after mounting
setTimeout(() => {
